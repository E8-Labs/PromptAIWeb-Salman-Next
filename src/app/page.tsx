...
import API_PATH from "app/lib/API_PATH";
  ...
  return (
    <main className="flex min-h-screen flex-col p-6" style={mainStyle}>
      ...
      <div className="bg-black rounded m-auto items-center justify-center flex flex-row h-2/5 w-3/5  shadow-lg">
        <div
          className="flex-grow flex flex-col justify-center gap-6 rounded bg-gray-50 px-6 py-10 md:w-2/4 h-40vh md:px-20  bg-appgreen"
          style={{ height: "70vh", backgroundColor: "#001812" }}
        >
          <Slider
            {...sliderSettings}
          ...
          >
            {sliderContent.map((slide, index) => (
              <div
                key={index}
                className="gap-4"
                style={{ height: "100vh", gap: "2rem", flexDirection: "column", flex: 1 }}
              >
        ...
        <div
          className="flex-grow flex items-center justify-center p-1 md:w-2/4  md:py-12 bg-black"
          style={{ height: "70vh" }}
        >
          <div className=" flex items-center h-full  p-2 justify-center " style={{ width: "100%" }}>
            {page === "intro" && (
              <ChatGptLogin
                signinBtnTapped={signinBtnTapped}
                registerBtnTapped={registerBtnTapped}
              />
            )}
            {page === "login" && (
              <LoginAI
