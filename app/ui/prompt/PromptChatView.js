...
const PromptChatView = (props) => {
    ...
    // [{id: 1, message: "Hello", from: "me"}, {id: 2, message: "Hi", from: "gpt"},
  ...
  const [canShowPromptHint, setCanShowPromptHint] = useState(0) // 0 can not show, 1 can show, 2 already showing
    ...
    useEffect(()=>{
        ...
        const sendFirstPromptForNewChat = async(user) => {
          console.log("Sending First Message " + prompt)
    ...
    const sendMessagesToServer = async (messagesToSend) =>{
        ...
        const u = JSON.parse(
          ...
          const config = {
            ...
            const data = { chatid: props.chat.id, messages: [
              {
          ...
          .then(data => {
              ...
              if (data.data.status){
                ...
                console.log("Error is here in send message")
              }
          })
          .catch(error => {
              console.log(error)
      ...
      const generateSummaryFromGPT = async (messageData)=>{
        const APIKEY = process.env.REACT_APP_CHATGPTKEY;
        console.log( APIKEY)
        const headers = {}
        
        const data = {
            model: process.env.REACT_APP_GPTModel,
            // temperature: 1.2,
            messages: messageData,
            max_tokens: 500,
        }
        const result = await axios.post(ApiPath.GPTCompletions, data, {
          headers: {
